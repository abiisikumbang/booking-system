                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-750">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($booking->status === 'pending')
                                <span class="px-2 py-1 text-xs font-semibold bg-yellow-100 text-yellow-800 rounded-full dark:bg-yellow-900 dark:text-yellow-200">Pending</span>
                            @elseif($booking->status === 'checked_in')
                                <span class="px-2 py-1 text-xs font-semibold bg-green-100 text-green-800 rounded-full dark:bg-green-900 dark:text-green-200">Selesai / Hadir</span>
                            @else
                                <span class="px-2 py-1 text-xs font-semibold bg-red-100 text-red-800 rounded-full dark:bg-red-900 dark:text-red-200">Batal</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            @if($booking->status === 'pending')
                                <button wire:click="checkIn({{ $booking->id }})" class="mr-2 px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-xs transition">
                                    Check In
                                </button>
